...
def mp_dict_mixed_worker(shared_dict: Any, worker_id: int, iterations: int, numpy_ratio: float, results_queue: Any) -> None:
    """Worker function for multiprocessing.Manager().dict() mixed data test"""
    print(f"  MP Dict Worker {worker_id}: Starting {iterations} mixed operations (numpy ratio: {numpy_ratio:.1f})...")
...
def shareddict_mixed_worker(worker_id: int, iterations: int, numpy_ratio: float, results_queue: Any) -> None:
    """Worker function for SharedDict mixed data test"""
    print(f"  SharedDict Worker {worker_id}: Starting {iterations} mixed operations (numpy ratio: {numpy_ratio:.1f})...")
...
def benchmark_mixed_data_performance() -> None:
    ...
    test_configs: list[dict[str, Any]] = [
        {"name": "Mostly Built-in (20% numpy)", "numpy_ratio": 0.2, "workers": 2, "iterations": 5},
        {"name": "Equal Mix (50% numpy)", "numpy_ratio": 0.5, "workers": 2, "iterations": 5},
        {"name": "Mostly Numpy (80% numpy)", "numpy_ratio": 0.8, "workers": 2, "iterations": 5},
        {"name": "All Numpy (100% numpy)", "numpy_ratio": 1.0, "workers": 2, "iterations": 3},
        {"name": "All Built-in (0% numpy)", "numpy_ratio": 0.0, "workers": 2, "iterations": 8},
    ]
    ...
    for config in test_configs:
        ...
        print(f"  Workers: {config['workers']}, Iterations per worker: {config['iterations']}")
        ...
        for worker_id in range(config['workers']):
            p = mp.Process(target=mp_dict_mixed_worker, 
                         args=(mp_dict, worker_id, config['iterations'], config['numpy_ratio'], mp_queue))
            p.start()
        ...
        d = SharedDict("mixed_benchmark", size=memory_mb*1024*1024, create=True, max_keys=512)
        ...
        sd_processes = []
        for worker_id in range(config['workers']):
            p = mp.Process(target=shareddict_mixed_worker, 
        ...
        if mp_times and sd_times:
            ...
            print(f"  MP Dict     - Store: {mp_avg_store*1000:.2f}ms, Retrieve: {mp_avg_retrieve*1000:.2f}ms, Total: {mp_total_time:.2f}s")
            print(f"  SharedDict  - Store: {sd_avg_store*1000:.2f}ms, Retrieve: {sd_avg_retrieve*1000:.2f}ms, Total: {sd_total_time:.2f}s")
            
            store_speedup = mp_avg_store / sd_avg_store if sd_avg_store > 0 else 0
            retrieve_speedup = mp_avg_retrieve / sd_avg_retrieve if sd_avg_retrieve > 0 else 0
            ...
            total_ops = len(sd_times) * 20 * 2  # 20 objects per iteration, store + retrieve
    ...
    for result in results_summary:
        ...
        print(f"  Throughput: {result['sd_ops_per_sec']:.0f} vs {result['mp_ops_per_sec']:.0f} ops/sec")
    ...
    for result in results_summary:
        ...
        print(f"  {numpy_pct:3.0f}% Numpy: {improvement:.2f}x faster - {performance_desc} improvement")
    
    # Overall averages
    avg_store_speedup = sum(r['store_speedup'] for r in results_summary) / len(results_summary)
    avg_retrieve_speedup = sum(r['retrieve_speedup'] for r in results_summary) / len(results_summary)
    avg_total_speedup = sum(r['total_speedup'] for r in results_summary) / len(results_summary)
    ...
    if avg_total_speedup > 1.5:
        print(f"  SharedDict shows strong performance advantages ({avg_total_speedup:.2f}x faster overall)")
        print("  across mixed workloads, with benefits for both numpy arrays and built-in types.")
        print("  The custom numpy serialization provides significant improvements even in mixed scenarios.")
    elif avg_total_speedup > 1.1:
        print(f"  SharedDict shows moderate performance improvements ({avg_total_speedup:.2f}x faster overall)")
        print("  in mixed data scenarios, with best results when numpy content is higher.")
    else:
