...
def mixed_dict() -> Generator[SharedDict, None, None]:
    ...
    d = SharedDict("mixed_types_fixture", size=50 * 1024 * 1024, create=True, max_keys=128)
    try:
        yield d
    finally:
        d.close()
...
def concurrent_dict() -> Generator[SharedDict, None, None]:
    ...
    d = SharedDict("mixed_concurrent_fixture", size=100 * 1024 * 1024, create=True, max_keys=256)
    try:
        yield d
    finally:
        d.close()
...
def performance_dict() -> Generator[SharedDict, None, None]:
    ...
    d = SharedDict("mixed_perf_fixture", size=100 * 1024 * 1024, create=True, max_keys=128)
    try:
        yield d
    finally:
        d.close()
...
def compatibility_dict() -> Generator[SharedDict, None, None]:
    ...
    d = SharedDict("compat_test_fixture", size=50 * 1024 * 1024, create=True, max_keys=64)
    try:
        yield d
    finally:
        d.close()
...
def test_mixed_data_types(mixed_dict: SharedDict) -> None:
    ...
    for key, original_value in test_data.items():
        ...
        if isinstance(original_value, np.ndarray):
            ...
            assert retrieved_value == original_value, f"Mismatch for key {key}: {retrieved_value} != {original_value}"
...
def test_mixed_data_concurrent_access(concurrent_dict: SharedDict) -> None:
    ...
    with mp.Pool(num_workers) as pool:
        ...
        for worker_id in range(num_workers):
            result = pool.apply_async(mixed_data_worker, (worker_id, "mixed_concurrent_fixture", iterations))
...
def test_numpy_builtin_serialization_compatibility(compatibility_dict: SharedDict) -> None:
